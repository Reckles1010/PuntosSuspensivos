...
¡Bienvenido al repositorio oficial de **Puntos Suspensivos**, un sistema CRM centralizado diseñado específicamente para optimizar la gestión operativa, el control de inventario y la administración de relaciones con clientes de una librería. Este proyecto integra de forma transversal los conocimientos de Programación, Bases de Datos, Lenguaje de Marcas y Entornos de Desarrollo de 1º de DAM.  El sistema permite gestionar de manera integral el stock de libros, sus respectivos autores, los clientes de la librería, los usuarios del sistema (vendedores) y la disponibilidad del inventario integrando Programación(Java), Base de Datos(Oracle PL/SQL y MySQL Server), Diseño Web(HTML, CSS, JavaScript). 
...
Verifica el archivo de propiedades o la clase de conexión (generalmente `ConexionBD.java` o `application.properties`) y activa/descomenta las credenciales del motor de base de datos que vayas a utilizar:
